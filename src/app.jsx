 ...
 * * SETUP INSTRUCTIONS:
 * 1. Ensure api/calendar.js is updated to the "Calendar App" version.
 * 2. Deploy and click the "Bug" icon in the top right to open Discovery Mode.
 * 3. Copy the numerical IDs for your rooms and paste them into 'pcoRoomId' below.
 */
...
const App = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Store raw rooms for the Bug/Discovery menu
      setApiRooms(result.rooms || []);

      // Calendar API returns "instances". We map them to our internal format.
      const mappedBookings = (result.instances || []).map(instance => {
        // Find the event title from the 'included' side-load
        const eventId = instance.relationships?.event?.data?.id;
        const eventData = (result.included || []).find(inc => inc.type === 'Event' && inc.id === eventId);
        
        // Find the room IDs associated with this instance
        ...
        return {
          ...
          pcoRoomIds: roomIds // Array of room IDs
        };
      });
      ...
      <main className="flex-1 overflow-hidden flex flex-col relative">
        {error && (
          <div className="bg-rose-50 border-b border-rose-100 p-4 flex flex-col items-center justify-center gap-1 text-rose-800 shrink-0">
            <div className="flex items-center gap-2 text-[10px] font-black uppercase"><AlertCircle size={14} /> System Notice</div>
                    ...
                    {bookings
                      ...
                      .map(b => (
                        <div 
                          ...
                          className={`absolute top-4 h-20 rounded-2xl p-4 shadow-lg border-l-4 border-white/30 text-white z-10 transition-transform hover:scale-[1.01] hover:z-20 flex flex-col justify-center ${CATEGORY_COLORS[room.category] || 'bg-indigo-600'}`}
                        >
                          <p className="text-[10px] font-black truncate uppercase leading-tight drop-shadow-sm">{b.title}</p>
                          <p className="text-[8px] font-bold opacity-80 uppercase mt-1.5 flex items-center gap-1.5">
                            <Clock size={10} className="shrink-0"/> {new Date(b.start).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}
                          </p>
                        </div>
                      ))}
                  </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t-4 border-amber-400 z-[100] h-96 overflow-hidden shadow-2xl flex flex-col p-8 animate-in slide-in-from-bottom-full duration-500">
            <div className="flex items-center justify-between mb-6">
              <div className="flex flex-col">
                  ...
                  <p className="text-[9px] font-bold text-amber-600/70 uppercase tracking-widest mt-1 italic">Map these IDs to INITIAL_ROOMS array</p>
              </div>
            ...
            <div className="flex-1 overflow-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-8">
              {apiRooms.length > 0 ? apiRooms.map(res => (
                <div key={res.id} className="p-4 rounded-2xl border-2 border-slate-100 bg-white flex items-center justify-between hover:border-amber-100 transition-all shadow-sm">
                  ...
                  <button onClick={() => { navigator.clipboard.writeText(res.id); alert(`ID ${res.id} copied!`); }} className="p-2.5 bg-slate-50 hover:bg-white rounded-xl border border-transparent hover:border-slate-200 text-slate-400 hover:text-indigo-600 transition-all"><CheckCircle2 size={18}/></button>
                </div>
              )) : (
                <div className="col-span-full py-16 flex flex-col items-center justify-center opacity-40">
                    <RefreshCw className="animate-spin mb-4" size={32} />
                    <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Searching Calendar Rooms...</p>
                </div>
              )}
            </div>
          </div>
