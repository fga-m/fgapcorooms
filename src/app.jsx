 ...
 * Features:
 * 1. Exact Multi-Alias Mapping: Support for precise technical names (e.g., FGA Melbourne vs FGAM).
 * 2. FGAM Room Mapping: Preserved specific addresses and technical identifiers.
 * 3. Navigation: "Today" reset and a sliding 8-hour window.
 * 4. Centered "NOW" Marker: Fixed clipping issues on the current time indicator.
 */
...
const INITIAL_ROOMS = [
  { 
    id: 'r1', 
    matchNames: [
      'FGAM Sanctuary - 38 Lexton Rd, Box Hill North, VIC 3129', 
      'FGA Melbourne - Sanctuary - 38 Lexton Rd, Box Hill North, VIC 3129'
    ], 
  ...
  { 
    id: 'r3', 
    matchNames: ['http://fgam.org.au/zoom'], 
    displayName: 'Zoom', 
    category: 'Admin', 
    capacity: 15 
  },
  { 
    id: 'r4', 
    matchNames: ['http://fgam.org.au/zoom2'], 
    displayName: 'Zoom 2', 
    category: 'Kids', 
    capacity: 80 
  },
  { 
    id: 'r5', 
    matchNames: ['http://fgam.org.au/zoom3'], 
    displayName: 'Zoom 3', 
    category: 'General', 
    capacity: 60 
  },
  { 
    id: 'r6', 
    matchNames: ['http://fgam.org.au/zoom4'], 
    displayName: 'Zoom 4', 
    category: 'Tech', 
    capacity: 8 
  },
  { 
    id: 'r7', 
    matchNames: ['FGAM Multipurpose Room - 38 Lexton Rd, Box Hill North, VIC 3129'], 
...
const App = () => {
  ...
  const processCalendarData = useCallback((rawData) => {
    ...
    const mappedBookings = parsedEvents.map((event, idx) => {
      // Find a room where the incoming location EXACTLY matches one of the aliases
      const room = INITIAL_ROOMS.find(r => {
        return r.matchNames.some(name => 
          event.location.toLowerCase().trim() === name.toLowerCase().trim()
        );
      });
  ...
  const fetchCalendar = useCallback(async () => {
    ...
    setError("Sync Error. Check your PCO link or network connection.");
    setIsLoading(false);
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-100 overflow-hidden">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                {bookings.filter(b => b.roomId).length} Assigned
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-hidden flex flex-col relative">
        {error && (
          <div className="bg-amber-50 border-b border-amber-200 p-3 flex items-center justify-center gap-2 text-amber-700 text-[10px] font-bold uppercase shrink-0 text-center">
              ...
              <div className="w-64 shrink-0 bg-slate-50/50 border-r border-slate-200 flex flex-col">
                <div className="h-14 border-b border-slate-200 flex items-center justify-between px-4 bg-slate-100/50">
                    <div className="flex flex-col">
                        <span className="uppercase tracking-widest text-[9px] font-black text-slate-400 leading-none">FGAM Rooms</span>
                ...
                {INITIAL_ROOMS.map(room => (
                  <button key={room.id} onClick={() => toggleRoomVisibility(room.id)} className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all ${selectedRoomIds.includes(room.id) ? 'bg-indigo-50 border-indigo-200 text-indigo-700 font-bold' : 'bg-white border-slate-100 text-slate-400 opacity-60'}`}>
                    <div className="text-left">
                      ...
                      <p className="text-[10px] font-black uppercase opacity-60 mt-1">{room.matchNames.length} Aliases Linked</p>
                    </div>
        ...
        {showDebug && (
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t-2 border-amber-200 z-[100] h-80 overflow-hidden shadow-2xl flex flex-col p-6">
            <div className="flex items-center justify-between mb-4"><h3 className="text-slate-800 font-black uppercase flex items-center gap-2"><Bug size={20} className="text-amber-600" /> Discover Technical Names</h3><button onClick={() => setShowDebug(false)}><X size={20}/></button></div>
            <div className="flex-1 overflow-auto grid grid-cols-1 md:grid-cols-2 gap-3">
              {discoveredLocations.map((loc, i) => {
                const isMapped = INITIAL_ROOMS.some(r => r.matchNames.some(name => loc.toLowerCase().trim() === name.toLowerCase().trim()));
                return (
